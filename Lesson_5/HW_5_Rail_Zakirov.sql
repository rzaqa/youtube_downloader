...
-- вычисляем возраст пользователей в годах
SELECT DATE_FORMAT(NOW(), '%Y') - DATE_FORMAT(updated_at, '%Y') FROM users;
-- вычисляем средний возраст из полученного результата
SELECT AVG(DATE_FORMAT(NOW(), '%Y') - DATE_FORMAT(updated_at, '%Y')) FROM USERS;
